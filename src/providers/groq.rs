...
impl GroqProvider {
    pub fn new() -> Self {
        Self {
            base_url: Arc::new(RwLock::new("https://api.groq.com/v1".to_string())),
...
impl Provider for GroqProvider {
    ...
    fn process_headers(&self, headers: &HeaderMap) -> Result<HeaderMap, AppError> {
        ...
        if let Some(api_key) = headers.get("x-groq-api-key") {
    ...
    async fn process_response(&self, response: Response<Body>) -> Result<Response<Body>, AppError> {
        ...
        {
            debug!("Processing Groq event stream response");
            ...
            headers.insert(
                ...
                HeaderValue::from_static("content-type, x-provider, x-groq-api-key"),
            );
            headers.insert(
